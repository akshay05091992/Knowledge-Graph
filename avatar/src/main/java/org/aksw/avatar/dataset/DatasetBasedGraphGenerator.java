...
public class DatasetBasedGraphGenerator {
	...
	private Set<String> blacklist = Sets.newHashSet("http://dbpedia.org/ontology/wikiPageExternalLink",
			...
			"http://dbpedia.org/ontology/wikiPageID", "http://dbpedia.org/ontology/wikiPageRevisionID",
			"http://dbpedia.org/ontology/wikiPageRedirects", "http://dbpedia.org/ontology/wikiPageDisambiguates",
	...
	public WeightedGraph generateGraph(OWLClass cls, double threshold, String namespace)
			throws NoGraphAvailableException {
	...
	public WeightedGraph generateGraph(OWLClass cls, double threshold, String namespace, Cooccurrence c)
		...
		final SortedSet<OWLObjectProperty> outgoingProperties = getMostProminentProperties(cls, threshold, namespace,
				Direction.OUTGOING);
		...
		LoadingCache<OWLObjectProperty, Node> property2Node = CacheBuilder.newBuilder()
				.maximumSize(allRelevantProperties.size()).build(new CacheLoader<OWLObjectProperty, Node>() {

					public Node load(OWLObjectProperty property) {
						return outgoingProperties.contains(property) ? new Node(property.toStringID())
								: new Node(property.toStringID(), false);
					}
				});
		...
		for (OWLObjectProperty prop1 : properties) {
			for (OWLObjectProperty prop2 : properties) {
				if (!prop1.equals(prop2) && !pair2Frequency.containsKey(pair = Sets.newHashSet(prop1, prop2))) {
					String query = "SELECT (COUNT(DISTINCT ?s) AS ?cnt) WHERE {" + "?s a <" + cls.toStringID() + ">."
							+ "?s <" + prop1.toStringID() + "> ?o1." + "?s <" + prop2.toStringID() + "> ?o2." + "}";
	...
	private Map<Set<OWLObjectProperty>, Double> getCooccurrences(OWLClass cls,
			Set<OWLObjectProperty> outgoingProperties, Set<OWLObjectProperty> incomingProperties) {
		Map<Set<OWLObjectProperty>, Double> pair2Frequency = new HashMap<Set<OWLObjectProperty>, Double>();
	...
	private Map<Set<OWLObjectProperty>, Double> getPropertySimilarities(OWLClass cls,
	...
	private SortedSet<OWLObjectProperty> getMostProminentProperties(OWLClass cls, double threshold, String namespace,
			Direction propertyDirection) {
		logger.info("Computing most prominent " + propertyDirection.name().toLowerCase() + " properties for class "
				+ cls + " ...");
	 ...
	 *            the useIncomingProperties to set
	 */
	...
	public void setIndiv(OWLIndividual in) {
		this.indiv = in;
	}
	...
	public static void main(String[] args) throws Exception {
		...
		WeightedGraph wg = graphGenerator.generateGraph(
				new OWLClassImpl(IRI.create("http://dbpedia.org/ontology/Organisation")), 0.2,
				"http://dbpedia.org/ontology/", Cooccurrence.PROPERTIES);
